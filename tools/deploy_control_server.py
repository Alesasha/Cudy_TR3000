...
def deploy(args: argparse.Namespace) -> dict[str, object]:
    ...
    try:
        ...
        output = ssh_exec(
            ...
            f"cp {shlex.quote(args.remote_dir + '/deploy/uswest/vpn-control-provider-refresh.service')} /etc/systemd/system/vpn-control-provider-refresh.service\n"
            f"cp {shlex.quote(args.remote_dir + '/deploy/uswest/vpn-control-provider-refresh.timer')} /etc/systemd/system/vpn-control-provider-refresh.timer\n"
