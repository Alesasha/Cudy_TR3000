...
def main() -> int:
    ...
    assert_contains(one_click, "will keep retrying in the background", label="one_click_install.sh")
    ...
    assert_contains(read_text(AGENT_DIR / "uninstall_systemd.sh"), "stop_singbox_transport.sh", label="uninstall_systemd.sh")
    assert_contains(read_text(AGENT_DIR / "stop_singbox_transport.sh"), 'pgrep -f "sing-box run -c transports/${name}\\\\.json"', label="stop_singbox_transport.sh")
    ...
    assert_contains(builder, "production smoke test failed", label="Build-LinuxAgentPackage.ps1")
    assert_contains(builder, "installed service will keep retrying", label="Build-LinuxAgentPackage.ps1")
