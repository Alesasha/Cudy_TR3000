...
def install_target(
    ...
    *,
    ...
    command = (
        ...
        f"cp {shlex.quote(posixpath.join(remote_dir, 'deploy/uswest/vpn-control-provider-refresh.service'))} "
        "/etc/systemd/system/vpn-control-provider-refresh.service\n"
        f"cp {shlex.quote(posixpath.join(remote_dir, 'deploy/uswest/vpn-control-provider-refresh.timer'))} "
        "/etc/systemd/system/vpn-control-provider-refresh.timer\n"
        "systemctl daemon-reload\n"
        f"systemctl enable --now {shlex.quote(service_name)}\n"
        "systemctl enable --now vpn-control-provider-refresh.timer\n"
