...
| OpenWrt router agent | The separate Go `cudy-router-agent` is deployed in `observe` mode. It fetches live/cached policy, renders managed dnsmasq/nft/transport files transactionally, preserves modes, and rolls back writes and transport service state on failure. Strict live checks require fresh policy, non-empty groups, and zero blockers. A guarded trial tool additionally arms an independent on-router rollback before the persistent apply gate can change. | `C:\Users\Alexander\sdk\go1.26.4\Go\bin\go.exe test ./cmd/cudy-router-agent`, `python tools\check_cudy_router_agent.py --strict`, `python tools\trial_cudy_router_agent_apply.py`, and `python tools\test_trial_cudy_router_agent_apply.py` | Run the first uncommitted guarded apply trial during a planned maintenance window, verify automatic rollback, then run a separately committed trial before main-router migration. |
