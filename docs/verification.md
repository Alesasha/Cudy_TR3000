...
| Android agent | Release `1.34 (35)` is built, signed and published on production with matching APK/manifest SHA256. Full IPv4 TUN capture, selective FakeIP DNS, protected direct/provider sockets, policy fetch, status posting, non-disruptive probes, authenticated verified update download, and persistent installed/latest version UI are implemented. Fresh installs accept a one-time code and receive a unique per-device key/token. Two physical phones run the agent and basic routing works. | `python tools\test_android_agent_ui.py`, `python tools\test_control_server_http.py`, Android Debug/Release builds, production restricted-SSH bootstrap smoke, APK/manifest SHA verification, a physical background update/install cycle, and a physical acknowledged update-dialog check | Complete the multi-day Wi-Fi/mobile/lock/reboot/update soak and add rendered probes. |
