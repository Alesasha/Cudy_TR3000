...
| Auto selection | Implemented as agent-side probes with fastest successful winner. Python and Go probes reject known geo-block response text and configured success/failure regexes. | `python tools\vpn_control_app.py auto-select example.com --candidates "proxyde, proxyus, uswest"`, `python tools\test_probe_semantics.py`, and `go test ./cmd/cudy-router-agent` | Add browser-rendered checks for JavaScript-only decisions and make dependent domains share one service winner. |
...
| Android agent | A rebuilt `1.19 (20)` test APK is enrolled and running on the physical phone. Full IPv4 TUN capture, DNS hijacking, protected direct/provider sockets, SSH control, policy fetch, status posting and probes are working. On 2026-07-15 Android marked the VPN `VALIDATED`; `example.com` used `Direct`, `chatgpt.com` used `proxynl`, and Telegram CIDRs used `proxynl`. Unchanged policy cycles kept the same libbox config hash. | `python tools\test_android_agent_ui.py`, `adb shell dumpsys connectivity`, and filtered `adb logcat` for `CudyAgent` | Increment/publish the release, reboot-test it, and add service dependency groups plus optional rendered probes for JavaScript-only geo decisions. |
