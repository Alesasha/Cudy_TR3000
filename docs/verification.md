...
| Android agent | Release `1.27 (28)` is built, signed and published on production with matching APK/manifest SHA256. Full IPv4 TUN capture, selective FakeIP DNS, protected direct/provider sockets, policy fetch, status posting and non-disruptive probes are implemented. Fresh installs accept a one-time code and receive a unique per-device key/token. Two physical phones run the current package and basic routing works. | `python tools\test_android_agent_ui.py`, `python tools\test_control_server_http.py`, Android Debug/Release builds, production restricted-SSH bootstrap smoke, and APK/manifest SHA verification | Complete the multi-day Wi-Fi/mobile/lock/reboot soak and add rendered probes. |
...
| OpenWrt router agent | The separate Go `cudy-router-agent` is deployed in `observe` mode. A guarded uncommitted route trial rolled back automatically, and a separate committed route trial retained generated files before returning to `observe`. Current strict checks report live policy, 31 routes, 5/5 critical services, zero blockers/warnings and zero transport actions. It still does not own DHCP or WAN. | `C:\Users\Alexander\sdk\go1.26.4\Go\bin\go.exe test ./cmd/cudy-router-agent`, `python tools\check_cudy_go_fallback.py --host 192.168.8.1 --strict`, and `python tools\check_cudy_router_agent.py --host 192.168.8.1 --expected-mode observe --strict` | Continue the observe soak and complete the Phase 6 physical/config rollback before main-router cutover. |
| Cudy main-router preflight | A fresh redacted read-only snapshot on 2026-07-18 produced no hard failure. AWG UDP 51830 and its WAN allow rule are present. VLAN 2, Cudy LAN migration, disabled/unencrypted Wi-Fi, six static forward targets and old AirTies host routes remain explicit review items. | `python tools\capture_cudy_preflight_snapshot.py`, `python tools\cudy_router_preflight.py`, `python tools\test_capture_cudy_preflight_snapshot.py` | Validate VLAN syntax, configure/test encrypted Wi-Fi, verify static target addresses and prepare independent rollback before moving cables or DHCP/WAN. |
