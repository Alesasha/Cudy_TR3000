...
| Android agent | Release `1.45 (46)` is built, signed and published on production with matching APK/manifest SHA256. Full IPv4 TUN capture, selective FakeIP DNS, protected direct/provider sockets, cached startup, persistent SSH control, policy fetch, status posting, non-disruptive probes, resumable authenticated update downloads with visible progress and strict no-downgrade reconciliation, explicit update states, and a protected mobile-admin screen are implemented. Fresh installs accept a one-time code and receive a unique per-device key/token. Service-probe warnings are bounded and isolated from core TUN recovery. | `python tools\test_android_agent_ui.py`, `python tools\test_control_server_http.py`, Android Release build, production APK/manifest SHA verification, ADB package/service inspection, and physical multi-cycle runs | Move the enrolled phones to 1.45, continue the multi-day Wi-Fi/mobile/lock/reboot soak, and add rendered probes. |
...
| Linux agent packaging | Production `1.30 (31)` is published. Background checks download and verify the package while the current service keeps running; installation requires explicit UI approval and then uses the staged archive. Its watchdog reports isolated service failures without stopping the agent and uses a temporary direct-routing cooldown, without disabling autostart, only after complete base-connectivity failure. Managed TUN interfaces are removed from `systemd-resolved` default DNS selection on every cycle. The taskbar icon tracks live health. | `python tools\test_linux_agent_packaging.py`, `python tools\test_linux_watchdog.py`, shell/Python syntax checks, and manifest/hash verification through the production update endpoint | Confirm Dima's transition to `1.30`, then complete the 24-48 hour suspend/resume, Wi-Fi, Zapret, UFW and update soak. |
