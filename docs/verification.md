...
| Android agent | Production `1.22 (23)` is enrolled, published and running on the physical MIUI test phone. Full IPv4 TUN capture, selective FakeIP DNS, protected direct/provider sockets, SSH control, policy fetch, status posting and non-disruptive production probes are working. A real reboot recovered control and a `VALIDATED` VPN; Direct and managed service checks passed without policy-cycle TUN reloads. | `python tools\test_android_agent_ui.py`, `python tools\test_probe_semantics.py`, `adb shell dumpsys connectivity`, filtered `adb logcat` for `CudyAgent`, and production manifest/SHA verification | Run a multi-day soak on a second daily-use phone, add protected admin-driven bootstrap provisioning, add rendered probes, and simplify the user UI. |
