...
| Android agent | Release `1.25 (26)` is built, signed and published on production with matching APK/manifest SHA256. Full IPv4 TUN capture, selective FakeIP DNS, protected direct/provider sockets, policy fetch, status posting and non-disruptive probes were verified on `1.24`. Fresh installs now accept only a one-time code: the APK's shared SSH key can reach only the enrollment listener, which returns a unique per-device key and token. A production end-to-end bootstrap/control-channel test passed. The in-app mobile admin implements authenticated user/device create, edit, enable/disable, delete and code creation. | `python tools\test_android_agent_ui.py`, `python tools\test_control_server_http.py`, Android Debug/Release builds, production restricted-SSH bootstrap smoke, `adb shell dumpsys activity services com.nashvpn.cudyagent`, and APK/manifest SHA verification | Install `1.25` on a fresh/reset physical phone, complete code-only activation, then run a multi-day soak and add rendered probes. |
