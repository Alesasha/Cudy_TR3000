...
| Android agent | Release `1.19 (20)` is enrolled and running on the physical phone. SSH control, policy fetch, libbox startup, status posting, selective routes, HTTP probes, and TCP probes through candidate exits were verified on 2026-07-13. The libbox platform exposes real Android interfaces and holds exactly one default-network callback across policy reloads. | `python tools\test_android_agent_ui.py`, `adb shell dumpsys connectivity`, and filtered `adb logcat` for `CudyAgent` | Implement loop-free full-TUN/direct-outbound handling so domain/SNI rules are actually captured; the failed full-route experiment was rolled back and was not published. |
