...
| Android agent | Release `1.44 (45)` is built, signed and published on production with matching APK/manifest SHA256. Full IPv4 TUN capture, selective FakeIP DNS, protected direct/provider sockets, cached startup, persistent SSH control, policy fetch, status posting, non-disruptive probes, resumable authenticated update downloads with visible progress and strict no-downgrade reconciliation, explicit update states, and a protected mobile-admin screen are implemented. Fresh installs accept a one-time code and receive a unique per-device key/token. | `python tools\test_android_agent_ui.py`, `python tools\test_control_server_http.py`, Android Release build, production APK/manifest SHA verification, ADB package/service inspection, and physical update runs | Complete Dima's one-time transition to 1.44, continue the multi-day Wi-Fi/mobile/lock/reboot soak on multiple phones, and add rendered probes. |
