...
| Android agent | Release candidate `1.24 (25)` is built, signed and installed on the physical MIUI test phone; the remote update catalog is still on the preceding deployed version. Full IPv4 TUN capture, selective FakeIP DNS, protected direct/provider sockets, per-device restricted SSH control, policy fetch, status posting and non-disruptive production probes are working. Admin-generated QR/file provisioning pins the SSH host key and does not embed a shared key in the APK. The in-app mobile admin implements authenticated user/device CRUD and enrollment over the active restricted SSH connection; its layout and secure login boundary are verified, while a complete live admin login and server deployment await recovery from the current uswest SSH banner incident. | `python tools\test_android_agent_ui.py`, `python tools\test_control_server_http.py`, Android Debug/Release builds, `adb shell dumpsys activity services com.nashvpn.cudyagent`, UIAutomator inspection, and local APK manifest/SHA verification | Deploy the server changes and `1.24` update manifest, complete a live mobile-admin CRUD/enrollment cycle, then run a multi-day soak on a second daily-use phone and add rendered probes. |
