...
| Android agent | Release `1.40 (41)` is built, signed and published on production with matching APK/manifest SHA256. Full IPv4 TUN capture, selective FakeIP DNS, protected direct/provider sockets, cached startup, persistent SSH control, policy fetch, status posting, non-disruptive probes, authenticated verified update download, and persistent installed/latest version UI are implemented. Fresh installs accept a one-time code and receive a unique per-device key/token. Two physical phones run the agent and basic routing works; the attached phone completed the user-approved `1.38 -> 1.40` installation and reports a foreground requested VPN service. | `python tools\test_android_agent_ui.py`, `python tools\test_control_server_http.py`, Android Release build, production APK/manifest SHA verification, ADB package/service inspection, and a physical `1.38 -> 1.40` installer run | Continue the multi-day Wi-Fi/mobile/lock/reboot soak on both phones and add rendered probes. |
| Windows desktop UI | Release `1.26 (27)` is published with a WinForms UI, installed shortcuts, asynchronous status/update checks, safe elevated Start/Stop, copyable diagnostics and automatic UI relaunch after package replacement. | `python tools\test_windows_agent_packaging.py`, PowerShell parser checks, source UI runtime smoke and extracted-package runtime smoke | Complete the physical Start/Stop/update/reboot acceptance with watchdog protection before enabling normal use on the development workstation. |
...
| Linux agent packaging | Production `1.25 (26)` is published. The update path verifies that systemd becomes active before reporting completion, and the watchdog repairs an enabled-but-inactive service. The one-click package includes UI/status/diagnostics, idempotent direct-route/DNS rollback and bundled sing-box runtime. | `python tools\test_linux_agent_packaging.py` and `powershell -ExecutionPolicy Bypass -File tools\Build-LinuxAgentPackage.ps1 -AgentId DC_via_Cudy-linux -IncludeRuntime` | Bring Dima's currently stale device online, confirm `1.25 (26)`, then complete the 24-48 hour suspend/resume, Wi-Fi, Zapret, UFW and update soak. |
