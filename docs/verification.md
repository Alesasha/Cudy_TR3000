...
| OpenWrt router agent | The separate Go `cudy-router-agent` is deployed in `observe` mode and does not own PBR, DHCP or WAN. The fallback runtime is healthy. Critical health is 5/5, blockers/warnings/transport actions are zero, three strict observe checks passed at five-minute intervals, and guarded preflight contains only eight PBR override changes. HTTP-proxy health checks use the upstream proxy from the root-only cached transport plan because router-local traffic cannot reliably validate the gVisor TUN. | `C:\Users\Alexander\sdk\go1.26.4\Go\bin\go.exe test ./cmd/cudy-router-agent`, `python tools\check_cudy_go_fallback.py --host 192.168.1.174 --strict`, `python tools\check_cudy_router_agent.py --host 192.168.1.174 --expected-mode observe --strict`, and `python tools\trial_cudy_router_agent_apply.py --host 192.168.1.174` | Run one guarded apply trial with the independent timed rollback; do not combine it with provider refresh or main-router cutover. |
