...
| Cudy main-router preflight | A fresh redacted read-only snapshot on 2026-07-18 produced no hard failure. AWG UDP 51830 and its WAN allow rule are present. The independent cutover guard is installed, disarmed, running and holds a SHA256-verified config backup. VLAN 2, Cudy LAN migration, disabled/unencrypted Wi-Fi, six static forward targets and old AirTies host routes remain explicit review items. | `python tools\capture_cudy_preflight_snapshot.py`, `python tools\cudy_router_preflight.py`, `python tools\test_capture_cudy_preflight_snapshot.py`, and `python tools\deploy_cudy_main_router_guard.py --check-only --json` | Validate VLAN syntax, configure/test encrypted Wi-Fi, verify static target addresses and prepare physical rollback before moving cables or DHCP/WAN. |
